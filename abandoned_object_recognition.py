...
while (cap.isOpened()):
    ...
    # frame_diff = cv2.bitwise_and(fgmask,fgmask)
    frame_diff = cv2.threshold(frame_diff, 200, 255, cv2.THRESH_BINARY)[1]
  
    #Canny Edge Detection
    edged = cv2.Canny(frame_diff,40,150) #any gradient between 30 and 150 are considered edges
    ...
    top_row = np.hstack((edged,thresh2))
            ...
            if cv2.contourArea(c) < 300 or cv2.contourArea(c)>20000:
                ...
                if sumcxcy in top_contour_dict:
                    if top_contour_dict[sumcxcy]>100:
                        ...
                        cv2.putText(frame,'%s'%('Abandoned Object'), (cx,cy),cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255),2)
    ...
    for i, j in obj_detected_dict.items():
        if frameno - obj_detected_dict[i]>500:
    ...
    bottom_row = np.hstack((original_frame, frame))
